...
// LEXER ------------------------------------------------- en este ejemplo no tendremos token_anotada y daremos directamente la lista de tokens en el c�digo
...
const token* espera(const token*& iter, auto pred){            // si lo que vemos cumple el predicado, avanzamos el iterador pero regresamos el que hab�a cumplido; en caso contrario es un error fatal
   if(!pred(*iter)){
...
const token* espera(const token*& iter, token esperada){       // si lo que vemos es el token que esperamos, avanzamos el iterador pero regresamos el que hab�a cumplido; en caso contrario es un error fatal
...
std::unique_ptr<expresion> parsea_expresion(const token*& iter){      // por referencia porque iremos avanzando
   ...
   */
   ...
   if (*iter == CORCHETE_IZQ) {                    // �tenemos corchete?
      ++iter;                                      // lo ignoramos
      auto expr = parsea_expresion(iter);          // jalamos la expresi�n interna de los corchetes
...
struct sentencia_ejecuta : sentencia {       // EJECUTA expresi�n
...
struct sentencia_if : sentencia {    // IF (expresi�n) { cero o m�s sentencias }
   std::unique_ptr<expresion> ex;
...
std::unique_ptr<sentencia> parsea_sentencia(const token*& iter) {
   ...
   */
   ...
   espera(iter, LLAVE_DER);      // ya sabemos que viene (sino el ciclo while no hubiera terminado), pero lo hacemos expl�cito
